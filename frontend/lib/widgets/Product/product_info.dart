...
class _ProductInfoState extends State<ProductInfo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Initialize animation controller for bouncing effect
    ...
    _animationController.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        setState(() {
          _showCartAnimation = false; // Hide animation after completion
        });
      }
    });
  }
  ...
  Future<void> _onAddToCart() async {
    ...
    final RenderBox renderBox = _cartButtonKey.currentContext!.findRenderObject() as RenderBox;
    final position = renderBox.localToGlobal(Offset.zero);
    setState(() {
      _cartButtonPosition = position;
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Container(
                    key: _cartButtonKey, // Add key to track button position
        ...
        if (_showCartAnimation)
          AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              // Start position: cart button
              final startX = _cartButtonPosition.dx;
              final startY = _cartButtonPosition.dy;
              // End position: top-right corner (cart icon)
              final endX = screenSize.width - 50; // Adjust based on your cart icon position
              final endY = 50.0; // Adjust based on your cart icon position

              // Interpolate position
              final currentX = startX + (endX - startX) * _animationController.value;
              final currentY = startY + (endY - startY) * _animationController.value - 50 * (1 - _bounceAnimation.value);

              return Positioned(
                left: currentX,
                top: currentY,
                child: Transform.scale(
                  scale: 1 + 0.3 * (1 - _bounceAnimation.value), // Bouncing scale effect
                  child: Container(
                    width: 40,
                    height: 40,
                    decoration: BoxDecoration(
